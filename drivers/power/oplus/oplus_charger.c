...
static void oplus_chg_gauge_update_check(struct oplus_chg_chip *chip, bool ffc_state);
...
static ssize_t proc_tbatt_pwroff_write(struct file *filp, const char __user *buf, size_t len, loff_t *data)
{
	...
	if (buffer[0] == '0') {
		...
		oplus_chg_track_set_gauge_ota_ctrl_status(true);
	} else if (buffer[0] == '5') {
		oplus_chg_track_set_gauge_ota_ctrl_status(false);
	}
...
static void oplus_chg_gauge_update_check(struct oplus_chg_chip *chip, bool ffc_state) {
	...
	if (chip->charger_type != POWER_SUPPLY_TYPE_USB_DCP) {
		chg_debug("type:%d is not the need\n", chip->charger_type);
		return;
	}
	if (chip->led_on == 1)
		return;
